...
\node[phase,
  ...
  (final) {Trained IDOR-Hunting Agent\\
           \scriptsize\texttt{dr4g0n369/idor-hunt-qwen3-4b-grpo-merged}};
...
\node[rectangle, rounded corners=5pt,
  ...
  (reward) {%
    ...
    \texttt{elif fp and env\_reward <= 0.05:}\\
    \quad\texttt{final = -0.2 \quad\quad\quad\# FP veto}\\
    \texttt{else:}\\
...
\node[rectangle, rounded corners=5pt,
  ...
  (vulns) {%
    \textcolor{accent1}{\textbf{Target Tasks}}\\[4pt]
    \textcolor{accent1}{$\bullet$} IDOR (Orders)\\
    \textcolor{accent1}{$\bullet$} IDOR (Docs)\\
    \textcolor{accent1}{$\bullet$} Privilege Escalation\\
    \textcolor{accent1}{$\bullet$} Access Control
  };
